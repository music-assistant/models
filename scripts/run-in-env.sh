...
# Activate pyenv and a virtualenv that provides the command if present, then run the command
...
# Skip a virtualenv without the command, such as the one `uv run` creates in a worktree with
